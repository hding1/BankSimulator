...
import GUI.monitor.SelectAccountMonitor;
...
//		this.add(balanceLabel);
		...
		for(int i = 0;i<c.getList().size();i++) {
			listModel.addElement((i+1)+".  "+ c.getList().get(i).getAccount()+String.format("%22.2f", c.getList().get(i).getAmount()));
		}
		...
		Button1.addActionListener(new SelectAccountMonitor(this));
		this.add(Button1);
